...
st.divider()
st.subheader("🫧 그래프 6. 개봉일 스크린 수와 총 관객 — 첫 주 관객 버블")
...
st.divider()
st.subheader("🔎 그래프 8. 10위권에 머문 날수와 총 관객의 관계")

st.write(
    "**질문: 10위권에 오래 머문 영화일수록 총 관객 수도 많은가?**"
)

graph8_df = df.copy()

graph8_df["days_in_top10_num"] = pd.to_numeric(
    graph8_df["days_in_top10"],
    errors="coerce"
)

graph8_df["total_audi_graph8"] = pd.to_numeric(
    graph8_df["total_audi"],
    errors="coerce"
)
...
fig8 = px.scatter(
    ...
    labels={
        "days_in_top10_num": "10위권에 머문 날수",
        "total_audi_graph8": "총 관객 수"
    }
)
...
st.info(
    "10위권에 머문 날수와 총 관객 사이에 "
    "어떤 관계가 있는지 산점도로 확인할 수 있습니다."
)
